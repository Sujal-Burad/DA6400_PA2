...
name = 'CartPole_type1_hyperparameter_tuning'
print("name = ", name)
...
parameters_dict = {
    'algorithm': {'values': ['dualing_dqn_type1']},
...
sweep_id = wandb.sweep(sweep_config, project="dualing_dqn_type1_cartpole_hyperparameter_finetuning_minimizing_regret")
...
def train(config=None):
    ...
    with wandb.init(config=config) as run:
        ...
        if config.algorithm == "dualing_dqn_type1":
            ...
            agent = Agent(state_size=state_shape, action_size=action_shape, seed=config.seed, device=torch.device("cuda" if torch.cuda.is_available() else "cpu"), adv_type="avg", BUFFER_SIZE=int(config.BUFFER_SIZE), BATCH_SIZE=config.BATCH_SIZE, LR=config.LR, GAMMA=config.GAMMA, UPDATE_EVERY=config.UPDATE_EVERY)
